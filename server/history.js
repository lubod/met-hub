...
function getDataStanica() {
    ...
    fetch('/getData/stanica/' + time).then(data => data.json()).then(json => {
        const last = json[0];
        const date = new Date(last.timestamp);
        document.getElementById('last1').innerHTML = `${date.toLocaleString('sk-SK')} Temperature ${last.temp} °C`;
        document.getElementById('last2').innerHTML = `${date.toLocaleString('sk-SK')} Humidity ${last.humidity} %`;
        document.getElementById('last3').innerHTML = `${date.toLocaleString('sk-SK')} Wind Speed ${last.windspeed} km/h`;
        document.getElementById('last4').innerHTML = `${date.toLocaleString('sk-SK')} Wind Gust ${last.windgust} km/h`;
        document.getElementById('last5').innerHTML = `${date.toLocaleString('sk-SK')} Wind Direction ${last.winddir} °`;
        document.getElementById('last6').innerHTML = `${date.toLocaleString('sk-SK')} Relative Pressure ${last.pressurerel} hPa`;
        document.getElementById('last7').innerHTML = `${date.toLocaleString('sk-SK')} Solar Radiation ${last.solarradiation} W/m2`;
        document.getElementById('last8').innerHTML = `${date.toLocaleString('sk-SK')} UV ${last.uv}`;
        document.getElementById('last9').innerHTML = `${date.toLocaleString('sk-SK')} Rain Rate ${last.rainrate} mm/hr`;
        document.getElementById('last10').innerHTML = `${date.toLocaleString('sk-SK')} Rain Event ${last.eventrain} mm`;
        document.getElementById('last11').innerHTML = `${date.toLocaleString('sk-SK')} Rain Hourly ${last.hourlyrain} mm`;
        draw_stanica(json);
    });
...
function draw_stanica(json_data) {
    ...
    for (var i in json_data) {
        result1.push([new Date(json_data[i].timestamp), parseFloat(json_data[i].temp)]);
        result2.push([new Date(json_data[i].timestamp), parseFloat(json_data[i].humidity)]);
        result3.push([new Date(json_data[i].timestamp), parseFloat(json_data[i].windspeed)]);
        result4.push([new Date(json_data[i].timestamp), parseFloat(json_data[i].windgust)]);
        result5.push([new Date(json_data[i].timestamp), parseFloat(json_data[i].winddir)]);
        result6.push([new Date(json_data[i].timestamp), parseFloat(json_data[i].pressurerel)]);
        result7.push([new Date(json_data[i].timestamp), parseFloat(json_data[i].solarradiation)]);
        result8.push([new Date(json_data[i].timestamp), parseFloat(json_data[i].uv)]);
        result9.push([new Date(json_data[i].timestamp), parseFloat(json_data[i].rainrate)]);
        result10.push([new Date(json_data[i].timestamp), parseFloat(json_data[i].eventrain)]);
        result11.push([new Date(json_data[i].timestamp), parseFloat(json_data[i].hourlyrain)]);
    }
    data1.addRows(result1);
    data2.addRows(result2);
    data3.addRows(result3);
    ...
    const chart3 = new google.visualization.LineChart(document.getElementById('chart_div3'));
    chart3.draw(data3, options);
    const chart4 = new google.visualization.LineChart(document.getElementById('chart_div4'));
    chart4.draw(data4, options);
    const chart5 = new google.visualization.ScatterChart(document.getElementById('chart_div5'));
    chart5.draw(data5, options);
    const chart6 = new google.visualization.LineChart(document.getElementById('chart_div6'));
    chart6.draw(data6, options);
    const chart7 = new google.visualization.LineChart(document.getElementById('chart_div7'));
    chart7.draw(data7, options);
    const chart8 = new google.visualization.ColumnChart(document.getElementById('chart_div8'));
    chart8.draw(data8, options);
    const chart9 = new google.visualization.ColumnChart(document.getElementById('chart_div9'));
    chart9.draw(data9, options);
    const chart10 = new google.visualization.ColumnChart(document.getElementById('chart_div10'));
    chart10.draw(data10, options);
    const chart11 = new google.visualization.ColumnChart(document.getElementById('chart_div11'));
